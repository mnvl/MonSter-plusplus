...
class BasicMotionEncoder(nn.Module):
    def __init__(self, args, radius, add_init=False):
        super(BasicMotionEncoder, self).__init__()
        self.args = args
        self.cor_planes = (8 + 1) * (2*radius + 1)
        self.convc1 = nn.Conv2d(self.cor_planes, 64, 1, padding=0)
...
class BasicMultiUpdateBlock(nn.Module):
    def __init__(self, args, hidden_dims=[], ndisps=[]):
        super().__init__()
        self.args = args
        self.encoder16 = BasicMotionEncoder(args, args.corr_radius[0])
        self.encoder8 = BasicMotionEncoder(args, args.corr_radius[1])
        self.encoder4 = BasicMotionEncoder(args, args.corr_radius[2])  #, add_init=True
    ...
    def forward(self, net, inp, corr=None, disp=None, disp_mono=None, iter04=True, iter08=True, iter16=True, update=True):
        ...
        if not iter04 and not iter08 and iter16 :
            ...
            motion_features = self.encoder8(disp, disp_mono, corr)  #disp_mono, 
            ...
            motion_features = self.encoder4(disp, disp_mono, corr)  #disp_mono, 
            # net[2] = self.gru16(net[2], *(inp[2]), pool2x(net[1]))
...
class BasicMultiUpdateBlock_mix2(nn.Module):
    def __init__(self, args, hidden_dims=[], ndisps=[]):
        super().__init__()
        self.args = args
        self.encoder = BasicMotionEncoder_mix2(args, args.corr_radius[2], ndisps[1])
        encoder_output_dim = 128
...
class BasicMotionEncoder_mix2(nn.Module):
    def __init__(self, args, radius, ndisp):
        super(BasicMotionEncoder_mix2, self).__init__()
        self.args = args
        cor_planes = 96 + (8 + 1) * (2*radius + 1)
        self.convc1 = nn.Conv2d(cor_planes, 64, 1, padding=0)
        self.convc2 = nn.Conv2d(64, 64, 3, padding=1)
