...
def valid_2_colormap_np(valid, cmap='rainbow'):

    valid = valid.cpu().detach().numpy().squeeze() 
    assert valid.ndim == 2

    valid_img = valid.astype(np.float32)

    norm = matplotlib.colors.Normalize(vmin=0, vmax=1)
    cmap_m = matplotlib.cm.get_cmap(cmap)
    map = matplotlib.cm.ScalarMappable(norm=norm, cmap=cmap_m)
    
    colormap = (map.to_rgba(valid_img)[:,:,:3] * 255).astype(np.uint8)
...
def exponential_decay_loss(disp_preds, disp_init_pred, disp_gt, valid, loss_gamma=0.9, max_disp=192):
    """ Loss function defined over sequence of flow predictions """

    loss_gamma = 0.9  # 权重衰减系数
    n_predictions = len(disp_preds)  # 总预测次数（1/16, 1/8, 1/4×2）
...
def get_batch_info(image_paths):
    return "\n".join([f"{pair}" for pair in image_paths])

@hydra.main(version_base=None, config_path='config', config_name='train_zeroshot')
def main(cfg):
    set_seed(cfg.seed)
    ...
    if not cfg.restore_ckpt.endswith("None"):
        ...
        total_step = 0
    train_loader, model, optimizer, lr_scheduler = accelerator.prepare(train_loader, model, optimizer, lr_scheduler)  #, val_loader
    ...
    try:
        while should_keep_training:
            ...
            for data in tqdm(active_train_loader, dynamic_ncols=True, disable=not accelerator.is_main_process):
                ...
                loss, metrics = exponential_decay_loss(disp_preds, disp_init_pred, disp_gt, valid, max_disp=cfg.max_disp)
                ...
                accelerator.log({'train/loss': loss_val, 'train/learning_rate': optimizer.param_groups[0]['lr']}, total_step) # log the learning rate of DPT_params
                ...
                if total_step % 100 == 0 and accelerator.is_main_process:
                    ...
                    # visualization GT warp to check if GT is read correctly
                    warped_right, _ = disp_warp(right, disp_gt)
                    valid_warp = valid.unsqueeze(1)  
                    warped_right_masked = warped_right * valid_warp
                    warped_img = torch.cat([warped_right_masked[0], left[0], right[0]], dim=1)
                    ...
                    valid_np = valid_2_colormap_np(valid[0].squeeze())

                    accelerator.log({"left": swanlab.Image(image1_np, caption="step:{}".format(total_step))}, total_step)
                    ...
                    accelerator.log({"valid_np": swanlab.Image(valid_np, caption="step:{}".format(total_step))}, total_step)
